...
if 'page' not in st.session_state or st.session_state.page is None:
    st.session_state.page = xata.query("comments", {"page": {"size": 1}})


st.write(st.session_state.page['records'])

if st.button("Next Page"):
    st.session_state.page  = xata.next_page("comments", st.session_state.page, pagesize=1)
    st.rerun()

if st.button("Prev Page"):
    st.session_state.page = xata.prev_page("comments", st.session_state.page, pagesize=1)
    st.rerun()
st.divider()
